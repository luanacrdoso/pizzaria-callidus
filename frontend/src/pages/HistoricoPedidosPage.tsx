...
async function buscarPedidosLocais() {
  ...
  const pedidos = await Promise.all(
    ids.map((id) =>
      fetch(`${API_URL}/pedidos/${id}`).then((r) =>
        r.ok ? r.json() : null
      )
    )
  );
...
async function buscarHistorico(): Promise<any[]> {
  const pedidosLocais = await buscarPedidosLocais();
  ...
  const pedidosConta = await r.json();

  const pedidosPorId = new Map<number, any>();

  pedidosConta.forEach((pedido: any) => {
    pedidosPorId.set(pedido.id, pedido);
  });

  pedidosLocais.forEach((pedido: any) => {
    pedidosPorId.set(pedido.id, pedido);
  });

  return Array.from(pedidosPorId.values()).sort(
    (a, b) =>
