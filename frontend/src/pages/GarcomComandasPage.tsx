...
async function buscarCardapioComoAny(): Promise<any[]> { const pizzas = await buscarPizzas(); return pizzas as any[]; }
const TAMANHOS = [{ chave: "brotinho", rotulo: "Brotinho" }, { chave: "media", rotulo: "Média" }, { chave: "grande", rotulo: "Grande" }];
...
export function GarcomComandasPage() {
  ...
  const { data: comandas } = useQuery({ queryKey: ["comandas", mesaId], queryFn: () => buscarComandas(Number(mesaId)), enabled: !!mesaId, refetchInterval: 5000 });
  ...
  const montarNomeEPreco = (): { nome: string; precoUnitario: number } | null => {
    ...
    if (produto.tipo === "sabor_unico") return { nome: produto.nome, precoUnitario: Number(produto[`preco_${tamanho}`] ?? 0) + precoExtras };
  ...
  const handleAdicionarItemNaLista = () => {
    ...
    setErro("");
    setItens([...itens, { pizzaId: produto.id, nome: resultado.nome, tamanho: produto.tipo === "combo" ? "-" : tamanho, extras: extrasSelecionados.map((e) => e.nome), observacoes, quantidade, precoUnitario: resultado.precoUnitario }]);
    limparItemAtual();
  };
    ...
    try {
      if (comandaAlvo === null) {
        ...
        await criarPedidoPresencial({
          tipo: "presencial", mesa_id: Number(mesaId), comanda_nome: novaComandaNome, garcom_username: obterUsernameFuncionario(),
          cliente_nome: novaComandaNome, itens, subtotal: subtotalNovosItens, taxa_entrega: 0, total: subtotalNovosItens + gorjeta,
          gorjeta_valor: gorjeta, forma_pagamento: "A definir",
        });
        setSucesso(`Comanda "${novaComandaNome}" aberta com sucesso!`); setNovaComandaNome(""); setIncluirGorjeta(false);
      } else {
        for (const item of itens) { await adicionarItemAoPedido(comandaAlvo, item); }
        setSucesso("Itens adicionados à comanda com sucesso!");
      }
      setItens([]); invalidarComandas();
  ...
  const handleFecharComanda = async (pedidoId: number, total: number, nomePagador: string) => {
    try {
      ...
      const resposta = await fetchComoFuncionario(`${import.meta.env.VITE_API_URL}/pedidos/${pedidoId}/status`, {
        method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "finalizado" }),
      });
      if (!resposta.ok) { const err = await resposta.json(); alert("Erro ao fechar comanda: " + (err.mensagem || "Erro desconhecido")); return; }
      invalidarComandas();
    } catch (e: any) { alert("Erro ao fechar comanda: " + e.message); }
  };
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto" }}>
      ...
      <div className="secao-checkout">
        ...
        <select value={mesaId} onChange={(e) => { setMesaId(e.target.value); setComandaAlvo(null); }} style={{ width: "100%" }}>
          <option value="">Clique para escolher a mesa...</option>{mesas?.map((m: any) => <option key={m.id} value={m.id}>Mesa {m.numero}</option>)}
        </select>
      </div>
      {mesaId && (
        <>
          <div className="secao-checkout">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              ...
              <button onClick={() => setComandaAlvo(null)} className={comandaAlvo === null ? "" : "btn-secundario"} style={{ fontSize: "0.85rem" }}>
                {comandaAlvo === null ? "● Criando nova comanda" : "+ Nova Comanda"}
              </button>
            </div>
            {(!comandas || comandas.length === 0) && <p className="subtext" style={{ padding: "12px 0" }}>Nenhuma comanda aberta nesta mesa no momento.</p>}
            <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 16 }}>
              {comandas?.map((c: any) => {
                const selecionada = comandaAlvo === c.id; const listaItens = c.itens ?? [];
                return (
                  <div key={c.id} className={`card-simples ${selecionada ? "card-destaque" : ""}`} style={{ padding: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <div>
                        <strong>👤 {c.comanda_nome}</strong>
                        {c.criado_em && (<span className="subtext" style={{ marginLeft: 8 }}>🕒 {new Date(c.criado_em).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</span>)}
                      </div>
                    ...
                    <div style={{ margin: "10px 0" }}>
                      ...
                      {listaItens.length === 0 ? (<p className="subtext" style={{ fontStyle: "italic", marginTop: 4 }}>Nenhum item registrado.</p>) : (
                        <ul style={{ listStyle: "none", paddingLeft: 0, margin: "8px 0 0" }}>
                          {listaItens.map((item: any, idx: number) => (
                            <li key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6, paddingBottom: 6, borderBottom: "1px dashed var(--border-soft)" }}>
                              <div>
                                <span style={{ fontWeight: 600 }}>{item.quantidade}x {item.nome}</span>{" "}
                                {item.tamanho && item.tamanho !== "-" && (<span className="subtext">({item.tamanho})</span>)}
                                {item.extras && (<div className="subtext" style={{ fontSize: "0.8rem", color: "var(--gold-dark)" }}>+ {Array.isArray(item.extras) ? item.extras.join(", ") : item.extras}</div>)}
                                {item.observacoes && (<div style={{ fontSize: "0.8rem", color: "var(--danger)" }}>Obs: {item.observacoes}</div>)}
                              </div>
                              <strong style={{ fontSize: "0.9rem" }}>R\$ {(Number(item.preco_unitario ?? 0) * Number(item.quantidade ?? 1)).toFixed(2)}</strong>
                            </li>
                          ))}
                        </ul>
                      )}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 12 }}>
                      <div><span className="subtext">Total Acumulado: </span><strong style={{ fontSize: "1.1rem", color: "var(--gold-dark)" }}>R\$ {Number(c.total).toFixed(2)}</strong></div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => setComandaAlvo(c.id)} disabled={selecionada} className={selecionada ? "" : "btn-secundario"} style={{ fontSize: "0.82rem" }}>
                          {selecionada ? "✓ Selecionada" : "+ Lançar Itens"}
                        </button>
                        <button onClick={() => handleFecharComanda(c.id, c.total, c.comanda_nome)} style={{ fontSize: "0.82rem", background: "var(--success)" }}>Fechar e Pagar</button>
                      </div>
                    </div>
                  </div>
                );
            ...
            {comandaAlvo === null && (
              <div style={{ padding: 16, background: "var(--cream-2)", borderRadius: "var(--radius-sm)" }}>
                <label className="campo-label" style={{ display: "block", marginBottom: 6 }}>Nome do Responsável pela Nova Comanda:</label>
                <input value={novaComandaNome} onChange={(e) => setNovaComandaNome(e.target.value)} placeholder="Ex: João Silva" style={{ width: "100%", marginBottom: 10 }} />
                <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                  <input type="checkbox" checked={incluirGorjeta} onChange={(e) => setIncluirGorjeta(e.target.checked)} /> Incluir 10% de gorjeta do garçom
                </label>
              </div>
            )}
          </div>
          <div className="secao-checkout">
            ...
            {mostrarSeletor && (
              <SeletorDeProduto 
                onSelecionar={(item) => {
                  setProdutoId(String(item.id));
                  setMostrarSeletor(false);
                }} 
                onFechar={() => setMostrarSeletor(false)} 
              />
            )}

            {produto && (
              <p style={{ fontWeight: 600, marginBottom: 8, color: "var(--gold-dark)" }}>
                Item selecionado: {produto.nome} ({produto.categoria})
              </p>
            )}
              ...
              <div className="opcoes-grid" style={{ marginBottom: 12 }}>
                {TAMANHOS.filter((t) => produto[`preco_${t.chave}`]).map((t) => (
                  <label key={t.chave} className={`opcao-card ${tamanho === t.chave ? "selecionada" : ""}`}>
                    <input type="radio" checked={tamanho === t.chave} onChange={() => setTamanho(t.chave)} /> {t.rotulo} (R\$ {Number(produto[`preco_${t.chave}`]).toFixed(2)})
                  </label>
                ))}
              </div>
            )}
            {produto && produto.tipo === "personalizavel" && detalhe && (
              <div style={{ marginBottom: 12 }}>
                ...
                <div className="opcoes-grid" style={{ marginBottom: 12 }}>
                  {TAMANHOS.filter((t) => produto[`preco_${t.chave}`]).map((t) => (
                    <label key={t.chave} className={`opcao-card ${tamanho === t.chave ? "selecionada" : ""}`}>
                      <input type="radio" checked={tamanho === t.chave} onChange={() => { setTamanho(t.chave); setSaboresSelecionados([]); }} /> {t.rotulo}
                    </label>
                  ))}
                </div>
                <label className="campo-label">Escolha até {detalhe[`max_sabores_${tamanho}`] ?? "—"} sabores:</label>
                <div className="opcoes-grid">
                  {(detalhe.sabores_permitidos ?? []).map((saborId: number) => {
                    const sabor = cardapio?.find((p: any) => p.id === saborId); if (!sabor) return null;
                    return (
                      <label key={saborId} className={`opcao-card ${saboresSelecionados.includes(saborId) ? "selecionada" : ""}`}>
                        <input type="checkbox" checked={saboresSelecionados.includes(saborId)} onChange={() => toggleSabor(saborId)} /> {sabor.nome}
                      </label>
                    );
                  })}
                </div>
                  ...
                  <div key={slotIndex} style={{ marginTop: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {Array.from({ length: slot.quantidade }).map((_, pos) => (
                        <select key={pos} value={escolhasCombo[slotIndex]?.[pos] ?? ""} onChange={(e) => atualizarEscolhaCombo(slotIndex, pos, e.target.value)}>
                          <option value="">Opção {pos + 1}</option>
                          {cardapio?.filter((p: any) => p.categoria_id === slot.categoria_id && p.tipo !== "combo").map((p: any) => (<option key={p.id} value={p.id}>{p.nome}</option>))}
                        </select>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="opcoes-grid">
                      {adicionais?.map((a: any) => (
                        <label key={a.id} className={`opcao-card ${extrasSelecionados.some((e) => e.nome === a.nome) ? "selecionada" : ""}`}>
                          <input type="checkbox" checked={extrasSelecionados.some((e) => e.nome === a.nome)} onChange={() => toggleExtra(a)} /> {a.nome} (+R\$ {Number(a.preco).toFixed(2)})
                        </label>
                      ))}
                    </div>
                  </div>
                )}
                <textarea placeholder="Observações do item (ex: sem cebola)..." value={observacoes} onChange={(e) => setObservacoes(e.target.value)} style={{ width: "100%", marginTop: 12 }} />
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 12 }}>
                  <div className="stepper-qtd">
                    <button onClick={() => setQuantidade(Math.max(1, quantidade - 1))}>-</button><span>{quantidade}</span>
          ...
          <div className="secao-checkout">
            ...
            <ul style={{ listStyle: "none", paddingLeft: 0, marginBottom: 16 }}>
              {itens.map((i, idx) => (
                <li key={idx} className="card-simples" style={{ padding: 12, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <button onClick={() => removerItemDaLista(idx)} className="btn-link" style={{ color: "var(--danger)" }}>Remover</button>
                  </div>
                </li>
              ))}
            </ul>
            {itens.length > 0 && (
              <div className="resumo-linha total" style={{ marginBottom: 16 }}><span>Subtotal destes itens:</span><span>R\$ {subtotalNovosItens.toFixed(2)}</span></div>
            )}
