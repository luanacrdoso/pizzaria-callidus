...
export function MotoboyMinhasEntregasPage() {
  ...
  const { data: entregas, isLoading } = useQuery({
    queryKey: ['minhas-entregas'], queryFn: buscarMinhasEntregas, refetchInterval: 8000
  });
 
  const mutationStatus = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) => atualizarStatusEntrega(id, status),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['minhas-entregas'] }),
  });
      ...
      <ul style={{ listStyle: "none", padding: 0 }}>
        {entregas?.map((p: any) => (
          <li key={p.id} style={{ marginBottom: 12, padding: 12, border: "1px solid #ddd", borderRadius: 6 }}>
            ...
            <a href={linkWhatsapp(p.cliente_telefone, p.id)} target="_blank" rel="noreferrer">Falar com o cliente</a>
