  ...
  const handleFinalizar = async () => {
    ...
    try {
      const pedido = await criarPedidoPresencial({
        ...
        cliente_nome: tipoEntrega === "presencial" ? `Mesa ${mesas?.find((m: any) => m.id === Number(mesaId))?.numero ?? ""}` : (nomeCliente || "Retirada balcão"),
    ...
    <div style={{ padding: 24, maxWidth: 600 }}>
      ...
      {tipoEntrega === "presencial" ? (
        <select value={mesaId} onChange={(e) => setMesaId(e.target.value)}>
          <option value="">Selecione a mesa</option>{mesas?.map((m: any) => <option key={m.id} value={m.id}>Mesa {m.numero}</option>)}
        </select>
      ) : (
          ...
          {(detalhe.sabores_permitidos ?? []).map((saborId: number) => {
            ...
            return (
              <label key={saborId} style={{ display: "block" }}>
                <input type="checkbox" checked={saboresSelecionados.includes(saborId)} onChange={() => toggleSabor(saborId)} /> {sabor.nome} (R\$ {Number(sabor[`preco_${tamanho}`] ?? 0).toFixed(2)} inteira)
              </label>
            );
          })}
        </div>
          ...
          {(detalhe.combo_slots ?? []).map((slot: any, slotIndex: number) => (
            <div key={slotIndex} style={{ marginBottom: 8 }}>
              ...
              {Array.from({ length: slot.quantidade }).map((_, pos) => (
                <select key={pos} value={escolhasCombo[slotIndex]?.[pos] ?? ""} onChange={(e) => atualizarEscolhaCombo(slotIndex, pos, e.target.value)} style={{ marginRight: 8, marginBottom: 4 }}>
      ...
      {produto && (
        <>
          ...
          <div style={{ marginTop: 8 }}>Quantidade: <input type="number" min={1} value={quantidade} onChange={(e) => setQuantidade(Number(e.target.value))} style={{ width: 60 }} /></div>
      ...
      <ul>
        {itens.map((i, idx) => (
          <li key={idx}>
            {i.quantidade}x {i.nome} {i.tamanho !== "-" && `(${i.tamanho})`} {i.extras.length > 0 && ` + ${i.extras.join(", ")}`} — R\$ {(i.precoUnitario * i.quantidade).toFixed(2)}
