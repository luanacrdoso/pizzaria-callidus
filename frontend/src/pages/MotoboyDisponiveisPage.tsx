...
export function MotoboyDisponiveisPage() {
  ...
  const { data: entregas, isLoading, isError } = useQuery({
    queryKey: ['entregas-disponiveis'], queryFn: buscarEntregasDisponiveis, refetchInterval: 8000
  });
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {entregas.length === 0 && <p>Nenhuma entrega chamada pela cozinha no momento.</p>}
      <ul style={{ listStyle: "none", padding: 0 }}>
        {entregas.map((p: any) => (
          <li key={p.id} style={{ marginBottom: 12, padding: 12, border: "1px solid #ddd", borderRadius: 6 }}>
            ...
            <p><strong>Total:</strong> R$ {p.total} — <strong>Taxa de entrega:</strong> R$ {p.taxa_entrega}</p>
            <ul>
              {(p.itens ?? []).map((i: any) => <li key={i.id}>{i.quantidade}x {i.nome} ({i.tamanho})</li>)}
            </ul>
            <button onClick={() => mutationAssumir.mutate(p.id)}>Assumir Entrega</button>
          </li>
        ))}
      </ul>
    </div>
