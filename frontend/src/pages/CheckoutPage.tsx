...
export function CheckoutPage() {
 ...
 itens: itens.map((i) => ({ pizzaId: i.pizzaId, nome: i.nome, tamanho: i.tamanho, extras:
i.extras, observacoes: i.observacoes, quantidade: i.quantidade, precoUnitario: i.precoUnitario
})),
 ...
 ? await fetchComoCliente(`${API_URL}/pedidos`, { method: "POST", headers: { "ContentType": "application/json" }, body: JSON.stringify(payload) })
 : await fetch(`${API_URL}/pedidos`, { method: "POST", headers: { "Content-Type":
"application/json" }, body: JSON.stringify(payload) });
 ...
 <input placeholder="Telefone" value={telefone} onChange={(e) =>
setTelefone(e.target.value)} />
 <input placeholder="Endereço de entrega" value={endereco} onChange={(e) =>
setEndereco(e.target.value)} />
 <select value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)}>
 <option value="">Forma de pagamento</option>
 {(config?.formas_pagamento_aceitas ?? []).map((f: string) => <option key={f}
value={f}>{f}</option>)}
 </select>
 <div>
 <input placeholder="Cupom" value={codigoCupom} onChange={(e) =>
setCodigoCupom(e.target.value)} />
 ...
 <button onClick={handleFinalizar} disabled={!formaPagamento || itens.length ===
0}>Finalizar Pedido</button>
 </div>
 );
}