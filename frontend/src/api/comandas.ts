...
export async function adicionarItemAoPedido(pedidoId: number, item: any) {
  const r = await fetchComoFuncionario(`${API_URL}/pedidos/${pedidoId}/itens`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      nome: item.nome, tamanho: item.tamanho, extras: item.extras, observacoes: item.observacoes,
      quantidade: item.quantidade, preco_unitario: item.precoUnitario, pizza_id: item.pizzaId,
    })
  });
...
export async function registrarPagamento(pedidoId: number, dados: { nome_pagador: string; valor_pago: number; forma_pagamento: string }) {
