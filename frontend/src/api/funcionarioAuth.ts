...
export async function loginFuncionario(username: string, password: string): Promise<{ token: string; cargo: string; nome: string }> {
...
export function salvarSessaoFuncionario(token: string, cargo: string, nome: string) {
...
export async function fetchComoFuncionario(url: string, options: RequestInit = {}): Promise<Response> {
...
export async function cadastrarFuncionario(dados: {
  username: string; senha: string; nome: string; telefone: string; cargo: string;
}): Promise<any> {
